...
sealed trait AutoGeneratedKey[T] {
  def value: T
}
case class AutoGeneratedShort(value: Short) extends AutoGeneratedKey[Short]
case class AutoGeneratedInt(value: Int) extends AutoGeneratedKey[Int]
case class AutoGeneratedLong(value: Long) extends AutoGeneratedKey[Long]

trait AutogenTable[K, E<:Entity[K]] extends Table[K,E] {
  def insertReturning(entity: E)
    (implicit dialect: SQLDialect, compK: Composite[K], compE: Composite[E]): ConnectionIO[K] = {
